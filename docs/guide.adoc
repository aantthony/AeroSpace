...
For now, only xref:commands.adoc#move-node-to-workspace[move-node-to-workspace], xref:commands.adoc#layout[layout], and xref:commands.adoc#split-to-focused[split-to-focused] commands are supported in the `run` callback.
You can combine `layout` with `split-to-focused automatic` to create i3-style splits based on the focused window's aspect ratio when a new window appears.
